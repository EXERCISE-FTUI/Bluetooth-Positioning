...
const User = () => {
	...
	const getPositionInBuilding = (latitude: number, longitude: number) => {
		...
		const buildingWidth =
			buildingCoords.topRight.longitude -
			buildingCoords.topLeft.longitude;
		const buildingHeight =
			buildingCoords.topLeft.latitude -
			buildingCoords.bottomLeft.latitude;

		const relativeX =
			((longitude - buildingCoords.topLeft.longitude) / buildingWidth) *
			100;
		const relativeY =
			((buildingCoords.topLeft.latitude - latitude) / buildingHeight) *
			100;
